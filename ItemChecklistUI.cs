...
namespace ItemChecklist
{
	class ItemChecklistUI : UIState
	{
		...
		public override void OnInitialize()
		{
			...
			muteButton = new UIToggleHoverImageButton(TextureAssets.MagicPixel.Value, ItemChecklist.instance.GetTexture("UIElements/closeButton"), "Toggle Messages", announce);
			...
			collectChestItemsButton = new UIToggleHoverImageButton(TextureAssets.MagicPixel.Value, ItemChecklist.instance.GetTexture("UIElements/closeButton"), "Toggle Collect Chest Items", collectChestItems);
			...
			showBadgeButton = new UIToggleHoverImageButton(TextureAssets.MagicPixel.Value, ItemChecklist.instance.GetTexture("UIElements/closeButton"), "Show Sort Value Text", showBadge);
			...
			FixedUIScrollbar checklistListScrollbar = new();
			...
			itemSlots = new UIElements.UIItemSlot[ItemLoader.ItemCount];
			Item[] itemSlotItems = new Item[ItemLoader.ItemCount];
			for (int i = 0; i < ItemLoader.ItemCount; i++)
			{
				itemSlots[i] = new UIElements.UIItemSlot(i);
				itemSlotItems[i] = itemSlots[i].item;
			}

			FieldInfo inventoryGlowHue = typeof(ItemSlot).GetField("inventoryGlowHue", BindingFlags.Static | BindingFlags.NonPublic)!;
			FieldInfo inventoryGlowTime = typeof(ItemSlot).GetField("inventoryGlowTime", BindingFlags.Static | BindingFlags.NonPublic)!;

			MethodInfo SortMethod = typeof(ItemSorting).GetMethod("Sort", BindingFlags.Static | BindingFlags.NonPublic)!;
			object[] parametersArray = { itemSlotItems, Array.Empty<int>() };
		...
		internal void UpdateCheckboxes()
		{
			...
			if (buttonsHaveDummyTextures)
			{
				...
				Main.instance.LoadItem(ItemID.ToxicFlask);
				...
				//Texture2D sortButtonTexture = Utilities.ResizeImage(TextureAssets.Item[ItemID.ToxicFlask].Value, 32, 32);
			...
			var temp = new List<UIElements.UIItemSlot>();
			for (int i = 0; i < itemChecklistPlayer.findableItems.Length; i++)
			{
				if (itemChecklistPlayer.findableItems[i])
				{
					...
					if ((showCompleted != 1 && itemChecklistPlayer.foundItem[i]) || (showCompleted != 2 && !itemChecklistPlayer.foundItem[i]))
					{
						...
						UIElements.UIItemSlot box = itemSlots[i];
			...
			if (lastfoundID > 0)
			{
				...
				{
					checklistGrid.Goto(element => element is UIElements.UIItemSlot itemSlot && itemSlot.id == lastfoundID, true);
				}
				lastfoundID = -1;
			}
		}

		private bool PassModFilter(UIElements.UIItemSlot itemSlot)
		{
			if (currentMod == 0)
				return true;
			if (currentMod == 1 && itemSlot.item.ModItem == null)
				return true;
			if (itemSlot.item.ModItem != null && itemSlot.item.ModItem.Mod.Name == modnames[currentMod])
				return true;
			return false;
		}
		...
		private void ValidateItemFilter()
		{
			...
			{
				bool found = itemSlots.Any(itemSlot => itemSlot.item.Name.Contains(itemNameFilter.currentString, StringComparison.OrdinalIgnoreCase));
				if (!found)
				{
					itemNameFilter.SetText(itemNameFilter.currentString[..^1]);
				}
			}
			updateNeeded = true;
		}
	...
	public class FixedUIScrollbar : UIScrollbar
	{
		protected override void DrawSelf(SpriteBatch spriteBatch)
		{
			...
			UserInterface.ActiveInstance = UISystem.ItemChecklistInterface;
